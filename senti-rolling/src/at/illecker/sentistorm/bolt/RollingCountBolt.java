...
import at.illecker.sentistorm.tools.NthLastModifiedTimeTracker;
import at.illecker.sentistorm.tools.SlidingWindowCounter;
import at.illecker.sentistorm.util.TupleHelpers;
...
public class RollingCountBolt extends BaseRichBolt {
  ...
  private boolean ranker; // indicates if the counter has to emit or not (to avoid the serialisation problem!)
  ...
  public RollingCountBolt(int windowLengthInSeconds, int emitFrequencyInSeconds, boolean ranker) {
	    this.windowLengthInSeconds = windowLengthInSeconds;
	    this.emitFrequencyInSeconds = emitFrequencyInSeconds;
	    counter = new SlidingWindowCounter<Object>(deriveNumWindowChunksFrom(this.windowLengthInSeconds,
	        this.emitFrequencyInSeconds));
